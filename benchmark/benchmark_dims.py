...
file_path = os.path.join(Path(os.path.abspath(__file__)).parents[2], "scratch", "DATASETS", "dims_data")
...
for dims in num_dims:
	...
	result_dictionary = run_algorithms(data, labels, str(numclus)+'_dims', result_dictionary, num_iterations, epsilon,
                                        dims, seed_array[run_counter])

	
	run_counter += 1

all_results_df = pd.DataFrame(result_dictionary)
all_results_df.rename(columns={"Num_clusters": "Num_Dims"})
