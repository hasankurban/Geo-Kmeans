...
int main(){
       ...
       ofstream avgresFile;
       string outFile = out_path + "Avg_output.csv" ;
       ...
       for(int i=0; i<file_list.size(); i++){
              ...
              for (int j = 0; j < num_clusters.size(); j++){
                     ...
                     vector<float> iters(5, 0);
                     vector<float> runTime(5, 0);
                     vector<float> runTime_per_iter(5, 0);
                     vector<float> dist_calcs(5, 0);
                     ...
                     for (int rep = 0 ; rep < num_rep ; rep++){
                            ...
                            auto ms_int2 = std::chrono::duration_cast<std::chrono::seconds>(t6 - t5);
                            
                            std::cout << "Total DCKmeans time: " << ms_int2.count() << " seconds\n";
                            
                            iters[rep] = res.loop_counter;
                            runTime[rep] = ms_int2.count();
                            runTime_per_iter[rep] = float(ms_int2.count()/res.loop_counter);
