...
float threshold, int num_iterations){
    ...
    {
        std::unique_lock<std::mutex> th_lock(mut);
        if(check_status.wait_for(th_lock, 60s) == std::cv_status::timeout) 
    ...
    //                                       {"birch20", 0.7778386}, {"birch30", 0.8218354} };
       ...
       for(int i=0; i<=file_list.size(); i++){
              ...
              for (int j = 0; j< num_clusters.size(); j++){
                    ...
                    ball_runtime_speed = 0, ball_dist_speed = 0, km_time = 0, km_dist = 0;
                    ...
                    if (km_time[data_list[i]+to_string(clus)] == -999){
                        ...
                        km_time = km_time[data_list[i]+to_string(clus)]; 
                    }
                    ...
                    if (km_dist[data_list[i]+to_string(clus)] == -999){
                        ...
                        km_dist = (dataset.size() * num_iters * clus);
                    }
                    else{
                        ...
                        km_dist = km_dist[data_list[i]+to_string(clus)];
                    }
                    ...
                    std::setprecision(2) << to_string(km_time) << "," << std::setprecision(6) <<
                    to_string(runTime_per_iter[0]) << "," << std::setprecision(2) << to_string(0)
                        << "," << std::setprecision(2) << to_string(km_dist[0]) <<
