...
class conv_kmeans{
    ...
    Tint kmeans(vector<vector <Tfloat> > &, 
    Tint, Tfloat, Tint, Tint , Tint, 
    vector<vector <Tfloat> > &, vector<Tint> &);

};

template <typename Tfloat, typename Tint>
Tint kmeans(vector<vector <Tfloat> > &dataset, 
Tint num_clusters, Tfloat threshold, Tint num_iterations, 
Tint numRows, Tint numCols, vector<vector <Tfloat> > &new_centroids, 
vector<Tint> &assigned_clusters){
    ...
    // float centroids[num_clusters][numCols];
    // float dist_matrix[dataset.size()][num_clusters];
