...
inline MatrixOur update_centroids_ann(MatrixOur &dataset, ClusterIndexVector &clusters_point_index, unsigned int k,
                                  unsigned int dataset_cols, VectorXb &flag,
                                  unsigned int iteration_counter, MatrixOur &old_centroids, VectorOur &x_centroids);

inline void update_radius_ann(MatrixOur &dataset, ClusterIndexVector &clusters_point_index, MatrixOur &new_centroids,
...
get_sorted_neighbors_ann(VectorOur &the_rs, MatrixOur &centers_dist, unsigned int now_ball, unsigned int k,
                     vector<unsigned int> &now_center_index, vector<unsigned int> &new_in_center_index);

inline void
cal_centers_dist_ann(MatrixOur &new_centroids, unsigned int iteration_counter, unsigned int k, VectorOur &the_rs,
                 VectorOur &delta, MatrixOur &centers_dist);

inline MatrixOur cal_dist_ann(MatrixOur &dataset, MatrixOur &centroids);

inline MatrixOur
cal_ring_dist_ann(unsigned int data_num, unsigned int dataset_cols, MatrixOur &dataset, MatrixOur &now_centers,
              vector<unsigned int> &now_data_index);

void initialize_ann(MatrixOur &dataset, MatrixOur &centroids, VectorOur &labels, ClusterIndexVector &clusters_point_index,
...
output_data run_ann(MatrixOur &dataset, MatrixOur &centroids, double thres=0.001, int iters = 100) {
    ...
    //initialize_ann clusters_point_index and point_center_dist
    initialize_ann(dataset, centroids, labels, clusters_point_index, clusters_neighbors_index, point_center_dist, lx,
    ...
    while (iteration_counter < iters) {
        ...
        new_centroids = update_centroids_ann(dataset, clusters_point_index, k, dataset_cols, flag, iteration_counter,
        ...
        if (check_convergence(new_centroids, old_centroids, thres) == false) {
            ...
            update_radius_ann(dataset, clusters_point_index, new_centroids, point_center_dist, the_rs, flag,
                          iteration_counter, cal_dist_num, k);
            //Calculate distance between centers

            cal_centers_dist_ann(new_centroids, iteration_counter, k, the_rs, delta, centers_dist);
            ...
            for (unsigned int now_ball = 0; now_ball < k; now_ball++) {
                ...
                SortedNeighbors neighbors = get_sorted_neighbors_ann(the_rs, centers_dist, now_ball, k,
...
MatrixOur load_data_ann(const char *filename) {
    /*
...
bool LessSort_ann(Neighbor a, Neighbor b) {
...
cal_centers_dist_ann(MatrixOur &new_centroids, unsigned int iteration_counter, unsigned int k, VectorOur &the_rs,
    ...
    */
    ...
    if (iteration_counter == 1) centers_dist = cal_dist_ann(new_centroids, new_centroids).array().sqrt();
    else {
        for (unsigned int i = 0; i < k; i++) {
            for (unsigned int j = 0; j < k; j++) {
...
void initialize_ann(MatrixOur &dataset, MatrixOur &centroids, VectorOur &labels, ClusterIndexVector &clusters_point_index,
    ...
    *Summary: initialize_ann related variables
    ...
    */
    ...
    MatrixOur M = cal_dist_ann(dataset, centroids);
    OurType secVal = -1;
